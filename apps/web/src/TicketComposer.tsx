...
import { ChevronDown, MessageSquarePlus, Mic, Paperclip, Pause, RotateCcw, Send, Sparkles, Trash2 } from 'lucide-react';
...
export function TicketComposer({ actor, onCreated }: { actor:Actor; onCreated:(ticketId:string,notice?:string)=>void }) {
  ...
  const sendTextMessage=async({collapse=true}:{collapse?:boolean}={})=>{
    ...
    const updated=await loadConversation(session.id);setIntake(updated);setForm(current=>({...current,description:updated.description}));setMessageDraft('');if(collapse)setComposerOpen(false);return updated;
  };
  const uploadConversationVoice=async(session:IntakeSession,voice:Recording,{collapse=true}:{collapse?:boolean}={})=>{
    ...
    await request(`/ticket-intakes/${session.id}/messages/${prepared.message.id}/voice/complete`,actor.session,actor.organizationId,{method:'POST'});const updated=await loadConversation(session.id);setIntake(updated);setRecording(null);setRecordingSeconds(0);if(collapse)setComposerOpen(false);return updated;
  };
  ...
  const runAi=async()=>{
    ...
    try{
      let session=await sendTextMessage({collapse:false});if(!session)session=await ensureConversation();
      const voice=recording?await compatibleRecording(recording):null;if(voice)session=await uploadConversationVoice(session,voice,{collapse:false});
      ...
      if(completed.status==='SUCCEEDED'){
        ...
        if([...applied.changedFields].some(field=>!['title','description','categoryId'].includes(field)))setDetailsOpen(true);setComposerOpen(false);setClarificationFor(null);
      }else{setPipeline('FAILED');setComposerOpen(true);setError(intakeFailureMessage(completed.lastErrorCode));}
    }catch(cause){
      if(cause instanceof DOMException&&cause.name==='AbortError')return;
      const message=cause instanceof Error?cause.message:'';setPipeline('FAILED');setComposerOpen(true);
  ...
  const openClarification=(issue:{id:string;summary:string;ticket?:Partial<TicketFormState>})=>{setClarificationFor({id:issue.id,title:issue.ticket?.title??issue.summary});setComposerOpen(true);setError('');globalThis.setTimeout(()=>descriptionRef.current?.focus(),0);};
  return <Card className="quick-ticket-card smart-ticket-composer"><div className="quick-ticket-heading"><div><p className="eyebrow">درخواست جدید</p><h2>درخواستتان را با زبان خودتان توضیح دهید</h2><p>ژوپیتر می‌تواند از روی متن یا صدای شما سایر فیلدها را پیشنهاد کند.</p></div><Sparkles aria-hidden="true"/></div>
    <form className="quick-ticket-form" onSubmit={submit}>
      ...
      {(!intake?.messages?.length||composerOpen)?<section className="message-composer" aria-label="نوشتن پیام درخواست">{clarificationFor&&<p className="clarification-context" role="status">توضیح دربارهٔ: <strong>{clarificationFor.title}</strong></p>}<label className="ticket-description-field"><span className="field-label-row"><span>پیام شما</span>{aiBadge('description')}</span><textarea ref={descriptionRef} maxLength={10000} value={messageDraft} onChange={event=>setMessageDraft(event.target.value)} placeholder="مشکل، زمان شروع و نتیجه‌ای که انتظار دارید را توضیح دهید…"/></label>
        ...
        </div></section>:<button className="add-message-button" type="button" onClick={()=>{setComposerOpen(true);globalThis.setTimeout(()=>descriptionRef.current?.focus(),0);}}><MessageSquarePlus size={18}/>افزودن توضیح یا پیام دیگر</button>}
