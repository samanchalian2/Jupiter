...
export function TicketComposer({ actor, onCreated }: { actor:Actor; onCreated:(ticketId:string,notice?:string)=>void }) {
  ...
  const sendTextMessage=async()=>{
    ...
    const updated=await loadConversation(session.id);setIntake(updated);setForm(current=>({...current,description:updated.description}));setMessageDraft('');setComposerOpen(false);return updated;
  };
  const uploadConversationVoice=async(session:IntakeSession,voice:Recording)=>{
    ...
    await request(`/ticket-intakes/${session.id}/messages/${prepared.message.id}/voice/complete`,actor.session,actor.organizationId,{method:'POST'});const updated=await loadConversation(session.id);setIntake(updated);setRecording(null);setRecordingSeconds(0);setComposerOpen(false);return updated;
  };
