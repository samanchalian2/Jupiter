...
describe('OpenAI-compatible HTTP provider', () => {
  ...
  it('requests the versioned ticket-intake schema and normalizes array maps', async () => {
    ...
    expect(result.output.contractVersion).toBe('ticket-intake.v4');expect(result.output.customFields).toEqual({asset:12});expect(result.output.confidenceByField).toEqual({title:.9,'customFields.asset':.8});expect(result.output.primaryIssue?.summary).toContain('پرینتر');
    const body=JSON.parse(String(fetchMock.mock.calls[0]![1]!.body));expect(body.response_format.json_schema.name).toBe('jupiter_ticket_intake_v4');expect(body.response_format.json_schema.strict).toBe(true);expect(body.messages[0].content).toContain('exactly one concise tag for each core dimension');expect(body.messages[0].content).toContain('self-corrections');
  });
});
