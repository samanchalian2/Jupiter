...
describe('AI provider contract', () => {
  it('requires structured, bounded analysis output', async () => {
    ...
    const result = await provider.analyze({ promptVersion: 'v1', text: 'redacted input', model: 'test-model' });
  ...
  it('keeps provider failures explicit for the queue worker to record', async () => {
    ...
    await expect(provider.analyze({ promptVersion: 'v1', text: 'safe', model: 'test-model' })).rejects.toThrow('provider unavailable');
  });
});
