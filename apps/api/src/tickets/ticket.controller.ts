...
export class TicketController {
  ...
  @Get('queue') async queue(@Query('status') status:string,@Query('priority') priority:string,@Query('q') query:string,@Query('sort') sort:string,@Query('page') page:string,@Query('pageSize') pageSize:string,@Headers('authorization') authorization?: string,@Headers('x-organization-id') organizationId?: string) { return this.tickets.page(await this.actors.fromHeaders(authorization, organizationId), {status,priority,query,sort,page:Number(page)||1,pageSize:Number(pageSize)||20}); }
