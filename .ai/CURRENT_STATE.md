...
**GOAL-059 — Authorized temporary IP preview (2026-09-26):** The complete local Jupiter dataset was copied into the isolated `jupiter` database on the authorized host and verified by migration and entity counts. API and worker run as dedicated system services under the limited `jupiter` OS/runtime roles; API binds only loopback and Nginx serves a temporary HTTP-only IP preview. The runtime role can perform platform/login reads while tenant work explicitly switches to `jupiter_app`, where RLS isolation was re-verified. No Arandi database, role, table, service or application configuration was changed. This is operational preview evidence only: canonical DNS/TLS, deployment secret manager, managed backup/restore, monitoring/alerting and the Windows/AD Connector host remain required for official GOAL-059 staging acceptance. Source and deployed checkout align at `e31d298`.

**Teal palette remediation (2026-09-15):** Jupiter's built-in primary is now `#014348`, with controlled Teal hover, active, soft and border tokens. The default logo and favicon use the same Teal mark. OCEAN/TEAL presets and explicit platform/organization Custom Primary values remain backward-compatible; semantic success/warning/danger colors remain independent. GOAL-059 remains blocked and is not changed by this local visual remediation.

**GOAL-059 resume (BLOCKED, 2026-09-16):** an authorized non-destructive server audit confirmed an existing Docker/Nginx/PostgreSQL host without a Jupiter deployment. Canonical `jupiter.pnsoffice.ir` has no usable DNS result from that host, so HTTPS, TLS and deployment were not attempted; the host also needs free-disk remediation before image builds. Protected connection data was not recorded. Evidence: `docs/GOAL_059_EVIDENCE.md`. GOAL-060 is not started.

**GOAL-059 — Staging Release Readiness & Deployment Gate Acceptance (BLOCKED, 2026-09-08):** audit identified source candidate `45bd3f10e988213a2f402e6b81f918a5cde044aa`, clean `main` and matching `origin/main` before evidence changes. No staging ingress/DNS/TLS, registry, secret manager, managed backup/restore, monitoring/alert channel or Windows/AD Connector host is available in this workspace; Docker is also unavailable. No local result is represented as staging PASS. Evidence: `docs/GOAL_059_EVIDENCE.md`. GOAL-060 is not started.

**GOAL-058 — Full End-to-End Business Acceptance (accepted, 2026-09-08):** supported three-tenant application/provisioning, owner-only concurrent Go-Live, role boundaries, complete ticket lifecycle, commercial/AI/Assist/Directory/appearance/Help/notification/audit isolation coverage and authenticated Persian RTL browser acceptance passed. A temporary-test cleanup-order defect was corrected; final DB integrity checks and fixture cleanup are zero. The isolated forward rehearsal completed migrations 001–056 (65 records); 117 API tests, 13 Web tests, typechecks, builds and diff check passed. Evidence: `docs/GOAL_058_EVIDENCE.md`. GOAL-059 is not started.

**GOAL-057 — Appearance Theme Migration & Custom Primary Validation (accepted after reset-semantics remediation, 2026-09-07):** nullable Platform/Organization Custom Primary persistence, server WCAG validation with deterministic white/dark `onPrimary`, runtime inheritance and Persian preview controls are complete. Platform «حذف رنگ سفارشی» now clears only `custom_primary` and preserves its selected preset, density, radius and logo; Help and integration coverage were corrected accordingly. Its historical `#315399` canonical color was superseded by the 2026-09-15 Teal palette remediation. Evidence: `docs/GOAL_057_EVIDENCE.md`.

**GOAL-056 — تکمیل محتوای راهنمای محصول (accepted, 2026-09-06):** کاتالوگ Help به ۱۵ مقالهٔ دامنه‌ای، registry محدود metadata، جست‌وجوی فارسی سبک، دسته‌بندی Help Center و publication runtime اختلاف‌محور ارتقا یافت. Trigger «راهنمای چرخهٔ تیکت» به Ticket Detail افزوده و anchor popover برای جلوگیری از overflow اصلاح شد. پذیرش نهایی Setup Wizard با سازمان موقت `SETUP` و عضویت مجاز `ORG_OWNER` از مسیر واقعی `/o/goal056-setup-acceptance` انجام شد؛ Trigger مقالهٔ منتشرشدهٔ `organization-setup-wizard` را در RTL و بدون document-level overflow در 375 و 1440 باز کرد و تمام داده‌های موقت حذف شدند. quality gateهای نهایی عبور کردند. Evidence: `docs/GOAL_056_EVIDENCE.md`. GOAL-057 آغاز نشده است.

**GOAL-055 — Organization Setup Wizard Completeness (accepted after canonical Go-Live remediation, 2026-09-03):** Setup organizations use a server-backed, versioned ten-step Wizard with owner-only atomic Go-Live. The legacy `tenant-setup/complete` compatibility endpoint delegates to the canonical Go-Live service; readiness is re-evaluated under lock so configuration drift blocks activation, and concurrent calls create exactly one successful lifecycle audit/event. Rejected readiness deliberately has no falsely claimed durable audit. Migrations through 055, 26 API files/99 tests, 2 Web files/11 tests, typechecks, production builds and diff checks passed. Evidence: `docs/GOAL_055_EVIDENCE.md`.

**GOAL-054 — Directory Connector Operational Completeness (accepted after re-pair remediation, 2026-09-03):** The outbound-only PowerShell/WinSW connector has derived operational health, lightweight versioned heartbeats, a command queue, `INCREMENTAL_SNAPSHOT` scheduling and daily batched Full reconciliation, versioned OU/group scope and allowed group-role mappings. Full absence is evaluated only after every announced batch arrives; incomplete or policy-changed Full reconciliation records `PARTIAL` without lifecycle action. A revoked Connector can now be explicitly re-paired by an owner/admin without a second record: old device credentials and stale pairing codes remain invalid, and successful pairing creates only a new device identity/token. Migrations, 98 API tests, 11 Web tests, typechecks and builds pass. Evidence: `docs/GOAL_054_EVIDENCE.md`.

**GOAL-053 — مدل بسته و ظرفیت Jupiter Assist (accepted after remediation; 375px browser-surface limitation remains, 2026-09-01):** ظرفیت Assist از شمارندهٔ policy به بسته‌ها، تخصیص‌های tenant-scoped و ledger immutable منتقل شد. پذیرش پرونده تنها محل settlement است؛ request، approval و queue مصرف ایجاد نمی‌کنند. اعلان تجاری فقط به `ORG_OWNER` با Membership canonical `active` و User فعال می‌رسد و event-window آن dedupe می‌شود. Platform می‌تواند تعریف package را با دلیل suspend/reactivate کند؛ تعلیق definition فقط تخصیص جدید را منع می‌کند و allocationهای صادرشده را تغییر نمی‌دهد. Owner فقط projection مجاز می‌بیند. Help فارسی runtime به نسخهٔ ۲ منتشر شد. Evidence: `docs/GOAL_053_EVIDENCE.md`.
...
**GOAL-050 — چرخهٔ عمر اشتراک تجاری (accepted after remediation, 2026-08-31):** Migrationهای 047/048 وضعیت‌های TRIAL، ACTIVE، PAST_DUE، SUSPENDED، CANCELLED و EXPIRED، timestampهای lifecycle و مهلت تجاری tenant-bound را افزودند. Transitionهای Platform Admin تحت قفل tenant، audit واقعی و dedupe اعلان مالک انجام می‌شود؛ worker انقضا/تعلیق را تکرارپذیر اجرا می‌کند. Smart Action و پذیرش جدید Assist پس از خروج از پنجرهٔ مجاز تجاری متوقف می‌شوند، اما ticketing دستی و پرونده‌های Assist پذیرفته‌شده ادامه دارند. remediation، پذیرش Assist را به resolver واقعی `JUPITER_ASSIST` متصل کرد؛ اشتراک AI نامرتبط دیگر کافی نیست. renewal مالک برای ACTIVE/PAST_DUE/EXPIRED tenant-bound است و graph/grace/AI/Assist/data-preservation/notification dedupe با 89 تست API پوشش دارد. پنل فارسی مالک وضعیت/مهلت را فقط می‌خواند و Platform Console کنترل transitionها را دارد. پرداخت، فاکتور و checkout اضافه نشده‌اند. Evidence: `docs/GOAL_050_EVIDENCE.md`.

**GOAL-049 — کنترل تجاری مالک و مصرف مازاد (remediation پذیرش، 2026-08-31):** Migration 046 سیاست overage، درخواست تجاری و dedupe اعلان را افزود. ترتیب رزرو advisory-lock دوره‌ای، بسته، اضطراری، overage و توقف است. تمدید فقط برای subscription قابل تمدید همان tenant ایجاد/اعمال می‌شود؛ درخواست مخدوش APPLIED نمی‌شود. تست‌های هدفمند cap، concurrent reservation، owner-only و renewal cross-tenant افزوده شده‌اند. پرداخت، فاکتور و checkout اضافه نشده‌اند.
