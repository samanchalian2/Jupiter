...
**GOAL-054 — Directory Connector Operational Completeness (in progress, 2026-09-02):** The existing outbound-only PowerShell/WinSW connector now has derived operational health, lightweight versioned heartbeats, a command queue, `INCREMENTAL_SNAPSHOT` scheduling and daily batched Full reconciliation, versioned OU/group scope and allowed group-role mappings. Full absence is evaluated only after every announced batch arrives; an incomplete Full records PARTIAL without lifecycle action. Sync runs retain counts/policy versions and conflicts are safe `PARTIAL` outcomes: safe records apply, identity conflicts are persisted for correction, and Directory grants never alter manual or owner/admin roles. Migrations, 96 API tests, 11 Web tests, typechecks, builds and runtime Help publication pass. Authenticated browser acceptance at 375/768/1024/1440 remains pending.
