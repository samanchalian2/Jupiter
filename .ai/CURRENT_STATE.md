...
**GOAL-057 — Appearance Theme Migration & Custom Primary Validation (accepted after reset-semantics remediation, 2026-09-07):** Jupiter Light Theme primary is `#315399`; nullable Platform/Organization Custom Primary persistence, server WCAG validation with deterministic white/dark `onPrimary`, runtime inheritance and Persian preview controls are complete. Platform «حذف رنگ سفارشی» now clears only `custom_primary` and preserves its selected preset, density, radius and logo; Help and integration coverage were corrected accordingly. Evidence: `docs/GOAL_057_EVIDENCE.md`.
