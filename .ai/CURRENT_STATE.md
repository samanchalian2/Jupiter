...
**GOAL-050 — چرخهٔ عمر اشتراک تجاری (complete, 2026-08-31):** Migrationهای 047/048 وضعیت‌های TRIAL، ACTIVE، PAST_DUE، SUSPENDED، CANCELLED و EXPIRED، timestampهای lifecycle و مهلت تجاری tenant-bound را افزودند. Transitionهای Platform Admin تحت قفل tenant، audit واقعی و dedupe اعلان مالک انجام می‌شود؛ worker انقضا/تعلیق را تکرارپذیر اجرا می‌کند. Smart Action و پذیرش جدید Assist پس از خروج از پنجرهٔ مجاز تجاری متوقف می‌شوند، اما ticketing دستی و پرونده‌های Assist پذیرفته‌شده ادامه دارند. پنل فارسی مالک وضعیت/مهلت را فقط می‌خواند و Platform Console کنترل transitionها را دارد. پرداخت، فاکتور و checkout اضافه نشده‌اند. Evidence: `docs/GOAL_050_EVIDENCE.md`.
