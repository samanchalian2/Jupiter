...
**GOAL-054 — Directory Connector Operational Completeness (complete, 2026-09-03):** The outbound-only PowerShell/WinSW connector has derived operational health, lightweight versioned heartbeats, a command queue, `INCREMENTAL_SNAPSHOT` scheduling and daily batched Full reconciliation, versioned OU/group scope and allowed group-role mappings. Full absence is evaluated only after every announced batch arrives; incomplete or policy-changed Full reconciliation records `PARTIAL` without lifecycle action. Sync runs retain counts/policy versions and conflicts are safe `PARTIAL` outcomes: safe records apply, identity conflicts are persisted for correction, and Directory grants never alter manual or owner/admin roles. Migrations, 97 API tests, 11 Web tests, typechecks, builds, runtime Help publication and authenticated browser acceptance at 375/768/1024/1440 pass. Evidence: `docs/GOAL_054_EVIDENCE.md`.
